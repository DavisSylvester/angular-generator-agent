...
export class Workspace {
  ...
  async saveCodeFile(runId: string, file: CodeFile): Promise<void> {
    const filePath = join(this.baseDir, runId, `output`, file.path);
    await mkdir(dirname(filePath), { recursive: true });
    await writeFile(filePath, file.content);
    this.logger.info(`Code file saved`, { path: file.path, type: file.fileType });
  }
  ...
  async saveIterationSnapshot(
    ...
    for (const file of files) {
      const fileName = basename(file.path) || `generated.ts`;
      await writeFile(join(iterDir, fileName), file.content);
    }
  }
